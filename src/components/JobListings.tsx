      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
            {careers.benefits.map((benefit, index) => {
              ...
              return (
                <div
                  key={benefit.title}
                  className="bg-white border border-brand-teal-100/40 rounded-sm p-5 hover:border-brand-gold-500/20 transition-colors"
                >
                  <div className="p-2 bg-brand-gold-500/10 text-brand-gold-600 rounded-sm w-fit mb-3">
        ...
        <div className="space-y-6">
          {careers.items.map((job, index) => (
            <motion.article
              ...
              className="bg-white border border-brand-teal-100/40 rounded-sm p-6 sm:p-8 hover:border-brand-gold-500/30 hover:shadow-md transition-all duration-300"
            >
              <div className="flex flex-col lg:flex-row lg:items-start lg:justify-between gap-6">
                <div className="space-y-4 flex-1">
                  <div className="flex flex-wrap items-center gap-3">
                    <span className="text-xs font-bold uppercase tracking-wider text-brand-gold-600 bg-brand-gold-500/10 border border-brand-gold-500/20 px-2.5 py-1 rounded-sm">
                      {job.department}
                    </span>
                      ...
                      <ul className="space-y-2">
                        {job.responsibilities.map((item) => (
                          <li key={item} className="flex gap-2 text-sm text-brand-teal-900/75 leading-relaxed">
                            <span className="w-1.5 h-1.5 rounded-full bg-brand-gold-500 shrink-0 mt-2" />
                            {item}
                          </li>
                        ))}
                      </ul>
                ...
                <div className="shrink-0 lg:pt-2">
                  <Link
                    ...
                    className="group inline-flex items-center gap-2 bg-brand-gold-500 text-brand-teal-950 font-bold px-6 py-3.5 rounded-sm hover:bg-brand-gold-400 active:scale-95 transition-all text-sm tracking-wide shadow-md shadow-brand-gold-500/10 w-full sm:w-auto justify-center"
                  >
                    Apply Now
        ...
        >
          ...
          <a
            ...
            className="inline-flex items-center gap-2 text-brand-gold-400 font-semibold hover:text-brand-gold-300 transition-colors text-sm"
          >
