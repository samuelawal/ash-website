      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
              ...
              {data.socials.map((social) => (
                <a
                  ...
                  {...(social.href.startsWith("http")
                    ...
                    : {})}
                  className="p-2.5 bg-white/5 hover:bg-brand-red-500 text-white/60 hover:text-white border border-white/10 hover:border-brand-red-500 rounded-sm transition-all duration-300 active:scale-95"
          ...
          <div className="lg:col-span-2 space-y-5">
            ...
            <ul className="space-y-3.5 text-sm">
              {data.links.company.map((link) => (
                <li key={link.name}>
                  <Link href={link.href} className="hover:text-brand-red-400 transition-colors">
                    {link.name}
                  </Link>
                </li>
              ))}
              ...
              {siteData.blog.items.slice(0, 2).map((post) => (
                <li key={post.title} className="group">
                  <Link href="#updates" className="block space-y-1.5">
                    <span className="block text-[11px] font-bold uppercase tracking-wider text-brand-red-500">
                      {post.category}
                    </span>
                    <span className="block text-sm font-semibold text-white/90 group-hover:text-brand-red-400 transition-colors line-clamp-2 leading-snug">
                      {post.title}
                    </span>
                  </Link>
                </li>
        ...
        <div className="pt-10 flex flex-col sm:flex-row justify-between items-center gap-4 text-xs text-brand-teal-200/50">
          ...
          <div className="flex gap-6">
            <Link href="#" className="hover:text-brand-red-400 transition-colors">
              Privacy Policy
            </Link>
            <Link href="#" className="hover:text-brand-red-400 transition-colors">
              Terms of Service
