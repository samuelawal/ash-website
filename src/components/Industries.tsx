    ...
    <section className="relative py-28 text-white bg-brand-teal-950 overflow-hidden select-none">
      ...
      <div className="absolute inset-0 z-0">
        <Image
          ...
          quality={80}
        />
        <div className="absolute inset-0 bg-gradient-to-t from-brand-teal-950 via-brand-teal-950/90 to-brand-teal-950/60"></div>
      </div>
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6 lg:gap-8">
          {data.list.map((item, index) => (
            <motion.div
              ...
              className="backdrop-blur-md bg-white/5 border border-white/10 hover:border-brand-gold-500/40 hover:bg-white/10 p-8 rounded-sm transition-all duration-300 group flex flex-col justify-between"
            >
              <div className="space-y-4">
                {/* Sector Identifier Badge */}
                <div className="flex justify-between items-center">
                  <div className="p-2.5 bg-brand-gold-500/10 border border-brand-gold-500/20 text-brand-gold-500 rounded-sm">
                ...
                {/* Name */}
                <h3 className="text-xl font-bold font-display text-white tracking-tight group-hover:text-brand-gold-400 transition-colors">
                  {item.name}
                </h3>
